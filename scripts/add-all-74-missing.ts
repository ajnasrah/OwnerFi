...
const missingProperties = JSON.parse(
  fs.readFileSync('/Users/abdullahabunasrah/Desktop/ownerfi/all-missing-properties.json', 'utf-8')
);

async function addAllMissingProperties() {
  console.log(`\n📤 Adding ${missingProperties.length} missing properties to Firebase...\n`);
  ...
  for (const prop of missingProperties) {
    ...
    const state = prop['State '] || prop['State'];
    const zipCode = prop['zip code '] || prop['zip code'];
    const price = parseFloat(prop['Price '] || prop['Price']) || 0;
    ...
    const balloonYears = parseFloat(prop['Balloon '] || prop['Balloon']) || null;
    const interestRate = parseFloat(prop['Interest rate '] || prop['Interest rate']) || 0;
    const downPaymentAmount = parseFloat(prop['down payment amount '] || prop['down payment amount']) || 0;
    ...
    try {
      ...
      console.log(`✅ ${added + 1}/${missingProperties.length}: ${address}, ${city}, ${state}`);
      added++;
    } catch (error: any) {
  ...
  console.log(`\n✅ Successfully added ${added}/${missingProperties.length} properties!`);
...
addAllMissingProperties().catch(console.error);
