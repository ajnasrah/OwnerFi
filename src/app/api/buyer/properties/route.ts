...
export async function GET(request: NextRequest) {
    return withErrorHandling(async () => {
      ...
      if (!searchCity || searchCity.length < 2) {
        throw ValidationErrors.invalidFormat('city', 'at least 2 characters');
      }

      if (!searchState || searchState.length !== 2) {
        throw ValidationErrors.invalidFormat('state', '2-letter state code');
      }
      ...
      try {
        ...
        throw new StandardizedApiError({
          ...
          message: 'Failed to query properties database',
          cause: firestoreError as Error,
          context: {
            function: 'buyer-properties-firestore-query',
            searchCity,
            searchState,
            fetchLimit
          }
    ...
    }, { endpoint: 'GET /api/buyer/properties' });
}