...
export default function InvestorDashboard() {
  ...
  }, [profile, activeFilter, sortBy, sortOrder, excludeLand, showHidden, currentPage, fetchKey]);

  // Reset page when filters change (handled via wrapper functions below)

  // Hide/unhide property (pass/unpass)
  const hideProperty = useCallback(async (dealId: string) => {
    const action = showHidden ? 'unpass' : 'pass';
    ...
    try {
      const res = await fetch('/api/buyer/pass-property', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ propertyId: dealId, action }),
      });
      if (!res.ok) {
    ...
    <div className="min-h-screen bg-slate-900">
      ...
      <main className="max-w-7xl mx-auto px-4 py-4 pb-20 md:pb-4">
        ...
        <div className="mt-3 flex items-center gap-2">
          <button
            onClick={() => { setShowHidden(prev => !prev); setCurrentPage(1); }}
          ...
          >
            ...
            {showHidden ? 'Showing Hidden Deals' : 'Show Hidden Deals'}
          </button>
          {showHidden && (
            <span className="text-xs text-slate-500">Click the X on a card to unhide it</span>
          )}
        </div>
          ...
          <div className="mt-16 text-center">
            {dealsError ? (
              ...
              <>
                ...
                <h2 className="text-xl font-bold text-white mb-2">No Hidden Deals</h2>
                <p className="text-slate-400 mb-4">
                  You haven&apos;t hidden any deals yet.
                </p>
                <button
